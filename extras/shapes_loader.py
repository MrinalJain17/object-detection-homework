...
class ShapeDataset(torch.utils.data.Dataset):
    ...
    * Shape dimensions: A tuple of values that define the shape size
    ...
    """
        ...
        return torch.from_numpy(np.asarray(image)).permute(2, 0, 1), target
    ...
    def get_gt(self):
        ...
        for img_id, d in enumerate(self.image_info):
            ...
            for (shape, color, dims) in d["shapes"]:
                assert shape in ["square", "circle", "triangle"]
                ...
                annotations.append(
                    {
                        ...
                        "bbox": [
                            float(bbox[0]),
                            float(bbox[1]),
                            float(bbox[2]) - float(bbox[0]) + 1,
                            float(bbox[3]) - float(bbox[1]) + 1,
